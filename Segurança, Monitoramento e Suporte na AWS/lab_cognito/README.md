# Roteiro do lab na atividade prática do DIO Live Coding
...
- Protocol - REST -> Create new API -> API name [lab_api_manu_cognito] -> Endpoint Type - Regional -> Create API
- Resources -> Actions -> Create Resource -> Resource Name [Items] -> Create Resource

### No Amazon DynamoDB

- DynamoDB Dashboard -> Tables -> Create table -> Table name [DIOItems] -> Partition key [id] -> Create table

### No AWS Lambda

#### Função para inserir item

- Lambda Dashboard -> Create function -> Name [put_items_function] -> Create function
- Inserir código da função ```put_items_function.js``` disponível na pasta ```/src``` -> Deploy
...
- Cognito Dashboard -> Manage User Pools -> Create a User Pool -> Pool name [UserPoolCognito]
